...
def _run_tick(store: InMemoryEventStore, tick: int) -> list:
...
def test_resource_shock_uses_fractional_carry_so_long_run_modifier_is_linear():
    ...
    assert sum(quantities) == 9  # 3 * 0.6 * 5, without truncation bias
    committed = _run_tick(store, 6)
    produced = next(event for event in committed if event.event_type == "resource.produced" and event.actor_id == "alice")
    assert produced.payload["stimulus_modifier"] == 0.0


def test_food_shortage_creates_hoarding_evidence_and_local_purchase():
    store = InMemoryEventStore()
    store.append_batch("main", [
        NewEvent(tick=0, phase="projection", event_type="world.created", payload={"flags": {"seed": "scarcity-test"}}),
        NewEvent(tick=0, phase="projection", event_type="entity.created", subject_ids=("buyer",), payload={"kind": "character", "components": {"position": {"location_id": "market"}, "health": {"current": 100, "maximum": 100}, "needs": {"hunger": 70, "fatigue": 0}, "survival": {"hunger": 70, "fatigue": 0}, "metabolism": {"hunger_per_tick": 0, "fatigue_per_tick": 0}, "inventory": {"food": 0}, "wallet": 10, "relationships": {"seller": 0}, "rumors": ["粮食可能会短缺"]}}),
        NewEvent(tick=0, phase="projection", event_type="entity.created", subject_ids=("seller",), payload={"kind": "character", "components": {"position": {"location_id": "market"}, "health": {"current": 100, "maximum": 100}, "needs": {"hunger": 0, "fatigue": 0}, "survival": {"hunger": 0, "fatigue": 0}, "metabolism": {"hunger_per_tick": 0, "fatigue_per_tick": 0}, "inventory": {"food": 10}, "wallet": 0, "relationships": {"buyer": 0}, "rumors": []}}),
    ], expected_sequence=0)
    committed = _run_tick(store, 1); world = replay_world(store.read("main")); types = [event.event_type for event in committed]
    assert world.entities["buyer"].components["inventory"]["food"] == 1
    assert world.entities["seller"].components["inventory"]["food"] == 9
    assert "scarcity.purchase" in types
    evidence = [event for event in committed if event.event_type == "decision.evidence" and event.actor_id == "buyer"]
    assert any(event.payload.get("decision") == "hoard_food" for event in evidence)


def test_spread_information_seeds_rumor_then_rumor_is_replay_safe():
    store = InMemoryEventStore(); _seed(store)
...
def test_high_scarcity_can_endogenously_trigger_resource_conflict():
    store = InMemoryEventStore()
    store.append_batch("main", [
    ...
    evidence = [event for event in committed if event.event_type == "decision.evidence" and event.actor_id == "hungry"]
