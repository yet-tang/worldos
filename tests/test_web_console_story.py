...
def test_story_console_keeps_summary_safety_and_adds_social_structure_surface(tmp_path):
    ...
    try:
        ...
        # Summary layer remains intact.
        assert "当前处境" in html
        assert "最近动态摘要" in html
        assert "function uiObservationDigest" in html

        # Story and social-structure layers are applied after humanization + summary.
        assert "性格与长期欲望" in html
        assert "function storyProfileSection" in html
        assert "function storySocialStructureSection" in html
        assert "社会关系与人情" in html
        assert "亲近" in html
        assert "信任" in html
        assert "怨气" in html
        assert "request_resource:'寻求帮助'" in html
        assert "help_resident:'帮助他人'" in html
        assert "repay_obligation:'兑现人情'" in html
        assert "reciprocity:'人情'" in html
        assert "social.interacted':'交往'" in html
        assert "obligation.defaulted':'失约'" in html
