...
class DeterministicTickEngine:
    ...
    def run_tick(self, timeline_id: str, tick: int) -> TickResult:
        ...
        for actor_id in actors:
            ...
            result = self.pipeline.process(
                timeline_id,
                intent,
                expected_sequence=len(self.store.read(timeline_id)),
            )
            ...
            action_events.extend(event for event in result.committed_events if event.phase in {"resolution", "projection"})
            ...
            if step_id and goal_id:
                ...
                self._append(
                    ...
                    [NewEvent(
                        ...
                        payload={"goal_id": goal_id, "step_id": step_id, "status": status},
                    )],
                    committed,
                    phase_counts,
                )
        ...
        if action_events:
            world = replay_world(self.store.read(timeline_id))
            perception_candidates = self.perception.derive(action_events, world)
        ...
        current_knowledge = self._knowledge_for_tick(knowledge, tick)
        memory_candidates = self.memory.derive(current_knowledge, tick=tick)
    ...
    def _append(
        ...
        if not candidates:
            return
        expected = len(self.store.read(timeline_id))
        appended = self.store.append_batch(timeline_id, candidates, expected_sequence=expected)
