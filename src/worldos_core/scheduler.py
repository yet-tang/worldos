...
from .knowledge import KnowledgeProjection, reduce_knowledge, replay_knowledge
from .memory import MemoryEngine, MemoryProjection, reduce_memory, replay_memory
...
from .world import WorldProjection, reduce_event, replay_world
...
class DeterministicTickEngine:
    ...
    def run_tick(self, timeline_id: str, tick: int) -> TickResult:
        ...
        self._append(cache, timeline_id, [NewEvent(tick=tick, phase="scheduler", event_type="tick.started", payload={"tick": tick})], committed, phase_counts)

        pre_context = ModuleContext(timeline_id=timeline_id, tick=tick, world=cache.world, history=tuple(cache.history))
        perception_state = cache.world.model_copy(deep=True)
        self._append(cache, timeline_id, self.modules.before_actions(pre_context), committed, phase_counts)

        self._append(cache, timeline_id, self.needs.derive(cache.world, cache.planning, tick=tick), committed, phase_counts)
        actors = tuple(sorted(owner_id for owner_id in cache.planning.goals_by_owner if cache.planning.active_goals(owner_id)))
        action_events: list[Event] = []

        for actor_id in actors:
            context = PlanningContext(owner_id=actor_id, tick=tick, world=cache.world, memory=cache.memory)
            ...
            if not cache.planning.pending_steps(goal.goal_id):
                ...
                self._append(cache, timeline_id, plan_events, committed, phase_counts)
            ...
            if step_id and goal_id:
                self._append(cache, timeline_id, [NewEvent(tick=tick, phase="planning", event_type="plan.step_status_changed", actor_id=actor_id, subject_ids=(actor_id,), correlation_id=goal_id, caused_by=tuple(event.event_id for event in result.committed_events), payload={"goal_id": goal_id, "step_id": step_id, "status": "completed" if result.accepted else "failed"})], committed, phase_counts)

        post_context = ModuleContext(timeline_id=timeline_id, tick=tick, world=cache.world, history=tuple(cache.history))
        self._append(cache, timeline_id, self.modules.after_actions(post_context, tuple(action_events)), committed, phase_counts)
        action_events.extend(event for event in committed if event.tick == tick and event.phase == "module")

        if action_events:
            self._append(cache, timeline_id, self.perception.derive(action_events, perception_state), committed, phase_counts)

        memory_candidates = self.memory.derive(self._knowledge_for_tick(cache.knowledge, tick), tick=tick)
        self._append(cache, timeline_id, memory_candidates, committed, phase_counts)
    ...
    def _append(self, cache: _ProjectionCache, timeline_id: str, candidates: list[NewEvent], committed: list[Event], phase_counts: dict[str, int]) -> None:
        if not candidates:
            return
        appended = self.store.append_batch(timeline_id, candidates, expected_sequence=len(cache.history))
        self._apply_committed(cache, appended, committed, phase_counts)

    @staticmethod
    def _apply_committed(cache: _ProjectionCache, appended: list[Event], committed: list[Event], phase_counts: dict[str, int]) -> None:
        for event in appended:
            cache.history.append(event)
            cache.world = reduce_event(cache.world, event)
            cache.planning = reduce_planning(cache.planning, event)
            cache.memory = reduce_memory(cache.memory, event)
            cache.knowledge = reduce_knowledge(cache.knowledge, event)
